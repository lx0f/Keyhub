...
manage_loyaltyprogram.get("/", async (req, res) => {
    ...
    res.render("./staff/loyaltyprogram/loyaltyprogram-table", {User_Cards,Users,Redeems,Vouchers});

});

manage_loyaltyprogram.post("/account/disabled/:id", async (req, res) => {

    const User_Card = await LoyaltyCard.findOne({ where: { id: req.params.id } });
    User_Card.update({
        Activation:"Off"
    })
